...
--#2
/*
The LAG function is used to show data from the preceding row or the table. When lining up rows the data is partitioned by country name 
and ordered by the data whn. That means that only data from Italy is considered.
...
--#8 The order seems weird, but others are corect
/*
For each country that has had at last 1000 new cases in a single day, show the date of the peak number of new cases.
*/
SELECT a.name, DATE_FORMAT(a.whn,'%Y-%m-%d'), MAX(a.new)AS peakNewCases
FROM (
SELECT c.name, c.whn, (c.confirmed - LAG(c.confirmed, 1) OVER (PARTITION BY c.name ORDER BY whn)) AS new
  FROM covid as c
ORDER BY new DESC
) AS a
WHERE a.new > 1000
GROUP BY a.name
ORDER BY DATE_FORMAT(a.whn,'%Y-%m-%d')
